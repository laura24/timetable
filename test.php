...
function timetable_find($room, $hour, $k) {
	global $DB;
    /**
     ...
     */
	 $crs=$DB->record_exists('timetable_base', array('hour'=>$hour, 'classroom'=>$room, 'day'=>$k));
	 echo $crs;

    if ( $DB->record_exists('timetable_base', array('hour'=>$hour, 'classroom'=>$room, 'day'=>$k))) 
	{
        return $DB->get_record('timetable_base', array('hour'=>$hour, 'classroom'=>$room,'day'=>$k));
...
function timetable_get_details($rec) {
    ...
    if (!empty($CFG->coursemanager)) {
        if ($data->cm) {
            // no set check, we're only interested in the teacher role
            $managerroles = split(',', $CFG->coursemanager);
            //some problems here, probably cause of role tinkering in other areas
...
function timetable_format_details($rec) {
    ...
    if (!$data->cm) {
        $output .= '<br>Bad coursemanager';
    }
    if (is_array($data->teachers) && count($data->teachers)) {                       //---------------------TEACHER
        $output .= '<br>';
        $index = 0;
...
function timetable_display() {
    global $CFG,$DB,$OUTPUT,$TIMETABLE_DAYS, $TIMETABLE_COLORS_PURE;
